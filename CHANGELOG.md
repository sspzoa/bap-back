...
- 프론트 — Next.js·이미지 처리 및 빌드 의존성의 알려진 취약점 패치 적용
- API — HTTP 클라이언트 의존성 undici의 보안 패치 적용
- 빌드 — Bun 잠금 파일로 설치를 고정하고 오래된 npm 잠금 파일 제거

## [2.1.4] - 2026-08-26

### Fixed

- 홈 — D-Flex 등 다시 고른 뒤에도 `/?home=1`이 쿠키를 무시하고 선택 화면을 보여 줌. 학교 선택은 `/?site=`로만 쿠키를 심음
...
- 홈 — `/?home=1`로 `bap-site-id` 쿠키를 서버에서 지워 사이트 선택 UI로 돌아가게 함
...
- 홈 — 오버레이·닫기·Esc 제거. 홈 진입 시 `bap-site-id` 쿠키 삭제

## [2.0.8] - 2026-08-26

### Added

- `/?site={id}` — 쿼리로 `bap-site-id` 쿠키를 넣고 해당 식단으로 진입
- 없는 경로 — `/`로 리다이렉트 (`/select` 전용 처리 제거)

## [2.0.7] - 2026-08-26

### Added

- 홈 하단 — API 문서 · 체인지로그 · 프론트/백엔드 기여하기 링크
